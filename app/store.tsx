...
const CityDataModel = types.model({
  latitude: types.maybeNull(types.string),
  longitude: types.maybeNull(types.string),
  data: types.frozen(),
});

type CityDataType = IModelType<typeof CityDataModel,>;

const CityData = CityDataModel.actions((self: CityDataType) => ({
  ...
  getUserLocation: flow(function* () {
    try {
      if (navigator && navigator.geolocation) {
        ...
        return self.fetchCityData(); // Return the result of fetchCityData
      } else {
...
const VisitedLocation = types.model({
  name: types.string,
...
const FavoriteLocation = types.model({
  name: types.string,
  latitude: types.number,
  longitude: types.number,
}).actions((self) => ({}));
...
const Store = types.model({
  ...
  fetchUserLocation: flow(function* () {
    const cityData = CityData.create();
    const userData = yield cityData.getUserLocation();
